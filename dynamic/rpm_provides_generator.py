#!/usr/bin/env python3
"""
This tool is used to build a list of what executables an rpm provides.
"""
        ...
        for rpm in rpm_list:
            ...
            for rpm_dict in list(rpm.keys()):
                ...
                for executable in executables:
                    ...
                    symbol_list = shared_obj["symbols"]
                    for symbol in symbol_list:
                        if not isinstance(symbol, dict):
                            print(shared_obj)
                            print(symbol_list)
                            raise Exception
                        if symbol["binding"] == "GLOBAL" and symbol["defined"] == "YES":
                            write_out("        " + symbol["symbol"])
                            if "at" in symbol:
                                write_out("@" + symbol["at"])
                            write_out('\n')
                    stdout.flush()

def load_search_and_print(json_dir, fil):
    #print(getcwd())
    #print(json_dir)
...
if __name__ == "__main__":
    ...
    load_search_and_print(args.json_directory, args.file)
